...
function game() {
  ...
  <aside class="round-log" aria-labelledby="rounds-title"><h2 id="rounds-title">Today’s marks</h2>${session.rounds.length ? `<ol>${session.rounds.slice(-8).reverse().map((round) => `<li><span class="log-shape">${round.count} ${motionLabel(round.count, round.motion)}</span></li>`).join('')}</ol>` : '<p class="empty">Completed rounds will appear here.</p>'}<p class="help">One shape mark appears for each completed motion.</p></aside></div>`;
}
