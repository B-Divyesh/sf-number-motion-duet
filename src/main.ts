import './style.css';
import heroArt from './assets/notebook-hero.webp';
...
function validRound(value: unknown): value is Round {
  return typeof value === 'object' && value !== null &&
    typeof (value as Round).count === 'number' && Number.isFinite((value as Round).count) &&
    (value as Round).count >= 1 && (value as Round).count <= 10 &&
    validMotion((value as Round).motion);
}
function storedSession(value: unknown): Session | null {
  if (typeof value !== 'object' || value === null) return null;
  const candidate = value as Partial<Session>;
  if (!validMotion(candidate.motion) || typeof candidate.count !== 'number' || !Number.isFinite(candidate.count) || !Array.isArray(candidate.rounds) || typeof candidate.confirmed !== 'boolean' || !candidate.rounds.every(validRound)) return null;
  return {
    motion: candidate.motion,
    count: Math.min(10, Math.max(1, Math.trunc(candidate.count))),
    rounds: candidate.rounds.map((round) => ({ count: Math.trunc(round.count), motion: round.motion })),
...
function readSession(): Session {
  ...
  try {
    ...
    try { localStorage.removeItem(key()); } catch { /* The recovery message is still useful if storage is unavailable. */ }
  }
...
function game() {
  ...
  const noun = motionLabel(session.count, session.motion);
  ...
  <aside class="round-log" aria-labelledby="rounds-title"><h2 id="rounds-title">Today’s marks</h2>${session.rounds.length ? `<ol>${session.rounds.slice(-8).reverse().map((round) => `<li><span class="log-shape">${round.count} ${motionLabel(round.count, round.motion)}</span></li>`).join('')}</ol>` : '<p class="empty">Completed rounds will appear here.</p>'}<p class="help">The shape marks show quantity without asking the child to read.</p></aside></div>`;
}
